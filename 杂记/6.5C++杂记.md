...
```cpp
/*通用的变量名字命名，不要太多缩写，小写加下划线 这个叫蛇形命名法*/
...
// 谷歌风格指南推荐
class MyClass {                    // 帕斯卡 - 类名
public:
    void MyMemberFunction();       // 帕斯卡 - 成员函数
    int my_member_variable;        // 蛇形 - 成员变量
};

void StandaloneFunction();         // 帕斯卡 - 独立函数
int global_variable;               // 蛇形 - 全局变量
